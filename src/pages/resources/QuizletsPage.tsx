  ...
  if (quizletId && activeQuizlet && step === 'browse') {
    return (
      <div className="min-h-screen bg-background">
        ...
        {/* Anonymous name prompt (shared-link landing) */}
        <AnimatePresence>
          {pendingQuizlet && (
            <motion.div
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 max-w-6xl py-10">
        <div className="mb-6 flex flex-wrap items-center gap-3">
          {user ? (
            <>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-accent/15 to-accent/5 border border-accent/30 rounded-full" title="Quiz attempts on consecutive days">
                <Flame size={14} className={(streak?.current_streak ?? 0) > 0 ? 'text-accent' : 'text-muted-foreground'} />
                <span className="text-xs font-bold text-primary tabular-nums">{streak?.current_streak ?? 0}-day streak</span>
              </div>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-muted/50 border border-border rounded-full" title="Your longest streak ever">
                <Medal size={14} className="text-muted-foreground" />
                <span className="text-xs text-muted-foreground tabular-nums">Best: {streak?.longest_streak ?? 0}</span>
              </div>
            </>
          ) : (
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-muted/30 border border-dashed border-border rounded-full">
              <Flame size={14} className="text-muted-foreground" />
              <span className="text-xs text-muted-foreground">Sign in to track your daily quiz streak</span>
            </div>
          )}
        </div>
