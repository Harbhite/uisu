...
import { ArrowLeft, BookOpen, PenTool, FileText, User, Mic, Book, Coffee, Feather, Filter, Newspaper, Quote, Plus, Loader2, Pencil } from 'lucide-react';
...
const InksVaultPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data, error } = await supabase
        .from('ink_pieces')
        .select('id, type, title, author_name, author_role, created_at, summary, user_id')
        .eq('is_published', true)
        .order('created_at', { ascending: false });

      if (!error && data) {
        setPieces(data as InksPiece[]);
      }
      setLoading(false);
    };
    fetchData();
  ...
  const renderCard = (piece: InksPiece) => {
    switch (piece.type) {
      ...
      case 'Report':
        return (
          <div onClick={() => navigate(`/inks-vault/piece/${piece.id}`)} className="bg-slate-50 rounded-lg border border-slate-200 hover:border-ui-blue/50 hover:shadow-md transition-all cursor-pointer h-full group overflow-hidden flex flex-col relative">
      ...
      case 'Article':
        return (
          <div onClick={() => navigate(`/inks-vault/piece/${piece.id}`)} className="bg-white rounded-none border-b-4 border-slate-200 hover:border-nobel-gold hover:bg-slate-50 transition-all cursor-pointer h-full group p-6 flex flex-col relative">
      ...
      case 'Blog':
        return (
          <div onClick={() => navigate(`/inks-vault/piece/${piece.id}`)} className="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all cursor-pointer h-full group overflow-hidden border border-slate-100 relative">
      ...
      case 'Essay':
        return (
          <div onClick={() => navigate(`/inks-vault/piece/${piece.id}`)} className="bg-[#FDFBF7] p-8 rounded-sm border border-stone-200 hover:border-stone-400 transition-all cursor-pointer h-full group flex flex-col shadow-[4px_4px_0px_0px_rgba(0,0,0,0.05)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] relative">
            <div className="mb-4">
      ...
      default:
        return (
          <div onClick={() => navigate(`/inks-vault/piece/${piece.id}`)} className="bg-white p-8 rounded-xl border border-slate-200 hover:shadow-lg transition-all cursor-pointer h-full group flex flex-col relative">
