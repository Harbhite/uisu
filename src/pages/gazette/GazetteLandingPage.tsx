  ...
  useEffect(() => {
    const fetchData = async () => {
      const [featuredRes, latestRes, issueRes] = await Promise.all([
        supabase.from('gazette_articles').select('*').eq('is_published', true).eq('is_featured', true).order('published_at', { ascending: false }).limit(3),
        supabase.from('gazette_articles').select('*').eq('is_published', true).order('published_at', { ascending: false }).limit(12),
        supabase.from('gazette_issues').select('*').eq('status', 'published').order('published_at', { ascending: false }).limit(1).maybeSingle(),
      ]);
    ...
    <div className="space-y-12">
      ...
      <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
        {heroArticle ? (
          ...
          <div className="bg-primary py-20 px-8 text-center relative overflow-hidden rounded-2xl">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        {latestIssue ? (
          <Link to={`/gazette/issues/${latestIssue.id}`}
            className="border border-accent/20 bg-accent/5 p-6 rounded-2xl flex flex-col justify-between gap-4 hover:bg-accent/10 transition-colors group"
          >
            <div className="space-y-2">
              ...
              <h3 className="text-xl font-serif font-bold text-foreground group-hover:text-accent transition-colors">{latestIssue.title}</h3>
            ...
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.15em] text-accent">Read Issue <ArrowRight size={12} /></div>
          </Link>
        ) : (
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.slice(0, 5).map((cat) => (
                <Link key={cat} to={`/gazette/category/${cat.toLowerCase().replace(/\s+/g, '-')}`}>
                  <Badge variant="outline" className="hover:bg-accent/10 hover:border-accent/30 transition-colors cursor-pointer text-[10px] rounded-full">{cat}</Badge>
                </Link>
              ))}
            </div>
          </div>
      ...
      <section>
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          {CATEGORIES.map((cat) => (
            <Link key={cat} to={`/gazette/category/${cat.toLowerCase().replace(/\s+/g, '-')}`}>
              <Badge variant="outline" className="px-5 py-2.5 hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all cursor-pointer text-xs font-bold uppercase tracking-wider rounded-full">{cat}</Badge>
            </Link>
          ))}
        </div>
      </section>
      ...
      {restArticles.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-foreground">The Latest</h2>
            <Link to="/gazette/category/all" className="text-[10px] font-bold uppercase tracking-[0.15em] text-accent hover:text-accent/80 transition-colors flex items-center gap-1">
      ...
      {/* Newsletter CTA */}
      <section className="grid md:grid-cols-2 gap-0 rounded-2xl overflow-hidden">
        <div className="bg-primary p-8 md:p-12 flex flex-col justify-center gap-3">
          <Mail size={20} className="text-accent" />
          <h3 className="text-xl md:text-2xl font-serif font-bold text-primary-foreground">Stay in the loop<span className="text-accent">.</span></h3>
          <p className="text-xs text-primary-foreground/50 leading-relaxed">Get the latest articles and official notices delivered to your inbox weekly.</p>
        </div>
        <Link to="/newsletter" className="bg-accent p-8 md:p-12 flex items-center justify-between group hover:bg-accent/90 transition-colors">
          <span className="text-sm font-bold uppercase tracking-[0.2em] text-accent-foreground">Subscribe</span>
