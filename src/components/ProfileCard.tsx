...
const ProfileCard: React.FC<ProfileCardProps> = ({
  avatarUrl = '/screenshots/index.png', // Default placeholder
  iconUrl, // Optional overlay icon
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      '--behind-glow-color': behindGlowColor ?? 'rgba(197, 160, 89, 0.4)', // Nobel Gold default
  ...
  // Complex styles that require CSS variables
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col items-start gap-0.5">
                      ...
                      <div className="text-xs text-white/70 leading-none">{name}</div>
                      <div className="text-[10px] text-nobel-gold/80 leading-none">{title} • {status}</div>
                    </div>
                  </div>
                  {onContactClick && (
                    <button
                      className="px-3 py-1.5 rounded-full bg-nobel-gold/80 hover:bg-nobel-gold text-white text-xs font-medium backdrop-blur-md transition-colors border border-white/10 ml-2"
                      onClick={(e) => {
                        e.stopPropagation();
                        handleContactClick();
                      }}
