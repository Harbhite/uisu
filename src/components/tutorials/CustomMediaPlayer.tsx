...
import {
  Play, Pause, Volume2, VolumeX, Maximize, Minimize, Settings, FastForward, Rewind
} from 'lucide-react';
...
const CustomMediaPlayer = ({ src, poster, type, className }: CustomMediaPlayerProps) => {
  ...
  const controlsTimeoutRef = useRef<NodeJS.Timeout | null>(null); // Fix #6
  ...
  return (
    ...
    >
      {type === 'video' ? (
        <video ref={mediaRef as React.RefObject<HTMLVideoElement>} src={src} poster={poster} className="w-full h-full object-contain" onClick={togglePlay} />
      ) : (
      ...
      {type === 'audio' && (
        <div className="absolute inset-0 flex items-center justify-center opacity-20 pointer-events-none">
           <div className="flex items-end gap-1 h-16">
              {[...Array(20)].map((_, i) => (
                <div key={i} className={cn("w-2 bg-white rounded-t-sm transition-all duration-100", isPlaying ? "animate-pulse" : "")}
      ...
      {!isPlaying && type === 'video' && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/40 z-10">
          <button onClick={togglePlay} className="w-20 h-20 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center hover:bg-[#6E5494] transition-all hover:scale-110">
          ...
          <div className="flex items-center gap-4">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="hover:text-purple-400 transition-colors flex items-center gap-1 text-xs font-bold">{playbackRate}x</button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="bg-black/90 border-white/20 text-white">
                {[0.5, 1, 1.25, 1.5, 2].map((rate) => (
                  <DropdownMenuItem key={rate} onClick={() => changePlaybackRate(rate)} className="focus:bg-purple-600 focus:text-white cursor-pointer">{rate}x</DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
            {type === 'video' && (
